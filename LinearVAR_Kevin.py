...
def learn_model(NE,z_data, A_l,eta,lamda): #TODO: make A, alpha, w, k, b optional
    ...
    for epoch in range(NE):  
        ...
        for t in range(P, T):   
            ...
            dC_dA,cost[t],cost_test[t],hat_z_t[:,t] = compute_gradients_l(z_data, A_l, t)

            A_l = A_l - eta*dC_dA - lamda * eta*np.sign(A_l)
            
        cost_history[epoch] = sum(cost)/(N*T*0.8)
        cost_history_test[epoch] = sum(cost_test)/(N*T*0.2)

    return cost_history,cost_history_test,A_l,hat_z_t




p_test = False #see that p_test is false important while comparing with linear and nonilnear
#it is important to keep p_test false. because both n and nonlinear need same z_data we are o
#only taking learn_model linear function from this file.
