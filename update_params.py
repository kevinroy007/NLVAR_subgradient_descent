...
def update_params(eta, z_data, A, alpha, w, k, b, t, z_range,lamda, newobject):
    ...
    if b_comparing:
        dC_dA, dc_dalpha, dc_dw, dc_dk, dc_db, cost,cost_test,hat_z_t = compute_gradients_compare( z_data, A, alpha, w, k, b, t,newobject)
    else:
        dC_dA, dc_dalpha, dc_dw, dc_dk, dc_db, cost,cost_test,hat_z_t = compute_gradients_n( z_data, A, alpha, w, k, b, t)
    ...
    w = w - eta* dc_dw
    k = k - eta* dc_dk
    #A  = A - eta*dC_dA - eta*lamda*A.sum()/abs(A).sum()
    A  = A - eta*dC_dA - eta*lamda*np.sign(A)    
    #pdb.set_trace()
    ...
    return A, alpha, w, k, b, cost,cost_test,hat_z_t
